...
// https://github.com/altmshfkgudtjr/react-epub-viewer?tab=readme-ov-file
...
export const Reader = () => {
  ...
  useEffect(() => {
    const createJapaneseOverlay = () => {
      const content = document.querySelector(".calibre");
      if (content) document.appendChild(content);
    };
    createJapaneseOverlay();
  });
  ...
  return (
    ...
    >
      <div style={{ margin: "0 10px" }}>
        ...
        <div>
          <button onClick={() => onPageMove("PREV")}>{"<"}</button>
          <button onClick={() => onPageMove("NEXT")}>{">"}</button>
        </div>
      </div>
    </div>
  );
